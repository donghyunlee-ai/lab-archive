---
layout: page
title: Alumni
permalink: /alumni/
nav: false
---

{% include figure.liquid loading="eager" path="assets/img/posts/post_2025-11-27.png" class="img-fluid rounded z-depth-1" zoomable=true %}

- **Sangwon Chae** (Data Analyst, **NH Bank**)

- **Sangmok Lee** (Data Analyst, **CJ ENM**, KRAFTON, Coinone)

- **Yunhwan Kim** (Data Analyst & Data Engineer, **Toss Payments**, Neowiz)

- **Suyeong Jang** (Data Analyst, **Toss Bank**, KRAFTON)

- **Geonu Park** (Data Analyst & Data Engineer, **Gravity (NASDAQ)**)

- **Seongjun Kwon** (Data Analyst & Data Engineer, **KB Bank**)

- **Jaehyeok Kim** (Investment Manager, **Daishin Securities**, eBest Investment Bank, Qraft(AI Quant))

- **Jaehun Park** (Data Analyst, **Hyundai Capital**)

- **Bumkeun Park** (Risk Manager, **Lotte Non-Life Insurance**)

- **Seho Hwang** (Data Analyst, **Penta System**)

- **Hyun Jung** (Data Analyst, **Toss Place**, Kakao Style, Kurly)

- **Junhyeok Shin** (Data Analyst, **Startup**)

- **Beomhui Lee** (Data Scientist, **Hanwha Ocean**)

- **Minkyu Kim** (Data Scientist)

- **Seungyeop Oh** (Data Scientist, **DeepX**)
